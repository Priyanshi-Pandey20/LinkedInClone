...
export default function Dashboard() {
  ...
  return (
    ...
    <DashboardLayout>
      ...
      <img className={styles.userProfile}src={`${clientServer.defaults.baseURL}/${authState.user?.userId?.profilePicture}`}/>
  ...
  console.log("FULL URL:", `${baseURL}/${post.media}`);
            return(
              <div key={post._id} className={styles.singleCard}>
                <div className={styles.singleCard__profileContainer}>
               
                  <img className={styles.userProfile} src={`${baseURL}/${post.userId?.profilePicture}`}/>
                  <div>
                    <div style={{display:"flex",gap:"1.2rem",justifyContent:"space-between"}}>
                  <p style={{fontWeight: "bold"}}>{post.userId?.name || "Unkown User"}</p>
